...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:si364@localhost/gardellaHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this. DONE
...
search_gifs = db.Table('search_gifs', db.Column('search_terms_id', db.Integer, db.ForeignKey('search_terms.id')), db.Column('gifs_id', db.Integer, db.ForeignKey('gifs.id')))
...
def get_gifs_from_giphy(search_string):
    ...
    result_list = []
    for g in json_data['data']:
        gifs = {}
        gifs['title'] = g['title']
        gifs['url'] = g['embed_url']
        result_list.append(gifs)
    return result_list
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    personal_gif_collection = PersonalGifCollection.query.filter_by(name = name, users = current_user.id).first()
    ...
    if personal_gif_collection:
        return personal_gif_collection
    ...
    # HINT: You can think of a PersonalGifCollection like a Playlist, and Gifs like Songs.
        ...
        personal_gif_collection = PersonalGifCollection(name = name, users = current_user.id, gifs = [])
        for g in gif_list:
            personal_gif_collection.gifs.append(g)
        db.session.add(personal_gif_collection)
        db.session.commit()
        return personal_gif_collection
...
def collections():
    ...
    collections = PersonalGifCollection.query.filter_by(users = current_user.id).all()
